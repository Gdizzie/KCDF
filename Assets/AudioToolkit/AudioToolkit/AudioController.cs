...
public class AudioController : SingletonMonoBehaviour<AudioController>
{
    ...
    /// </summary>
    public const string AUDIO_TOOLKIT_VERSION = "5.1";
    ...
    /// </summary>
    /// <param name="audioID">The audio ID.</param>
    /// <param name="fadeOutLength">The fade out time.</param>
    ...
    protected AudioObject _Play( string audioID, float volume, Vector3 worldPosition, Transform parentObj, float delay, float startTime, bool playWithoutAudioObject )
    {
        ...
        return PlayAudioItem( sndItem, volume, worldPosition, parentObj, delay, startTime, playWithoutAudioObject, null );
    }
    ...
    /// </returns>
    public AudioObject PlayAudioItem( AudioItem sndItem, float volume, Vector3 worldPosition, Transform parentObj, float delay, float startTime, bool playWithoutAudioObject, AudioObject useExistingAudioObj )
    {
        ...
        foreach ( var sndSubItem in sndSubItems )
        {
            if ( sndSubItem != null )
            {
                var audioObjRet = PlayAudioSubItem( sndSubItem, volume, worldPosition, parentObj, delay, startTime, playWithoutAudioObject, useExistingAudioObj );
                ...
                if ( audioObjRet )
                {
                    ...
                    {
                        audioObjRet._audioSource_MinDistance_Saved = audioObjRet.GetComponent<AudioSource>().minDistance;
                        audioObjRet._audioSource_MaxDistance_Saved = audioObjRet.GetComponent<AudioSource>().maxDistance;
                        audioObjRet.GetComponent<AudioSource>().minDistance = sndItem.audioSource_MinDistance;
                        audioObjRet.GetComponent<AudioSource>().maxDistance = sndItem.audioSource_MaxDistance;
                    }
                }
            }
        }
    ...
    protected static AudioSubItem[ ] _ChooseSubItems( AudioItem audioItem, AudioPickSubItemMode pickMode, AudioObject useExistingAudioObj )
    {
        ...
        if ( !object.ReferenceEquals( useExistingAudioObj, null ) ) // don't use Unity's operator here, because it will be called by GaplessAudioClipTransistion
        {
    ...
    /// The created <see cref="AudioObject"/> or <c>null</c>
    /// </returns>
    public AudioObject PlayAudioSubItem( AudioSubItem subItem, float volume, Vector3 worldPosition, Transform parentObj, float delay, float startTime, bool playWithoutAudioObject, AudioObject useExistingAudioObj )
    {
        ...
        if ( audioItem.Loop == AudioItem.LoopMode.LoopSequenceGapless )
        {
#if UNITY_FLASH
            Debug.LogError( "Gapless looping not supported by Flash" );
            return null;
#else
            sndObj._CreateGaplessAudioClipTransition( subItem );
#endif
        }
        else
        {
            sndObj.GetComponent<AudioSource>().clip = subItem.Clip;
            audioObjInstance.name = "AudioObject:" + sndObj.GetComponent<AudioSource>().clip.name;
        }
        
        sndObj.GetComponent<AudioSource>().pitch = AudioObject.TransformPitch( subItem.PitchShift );
        sndObj.GetComponent<AudioSource>().panStereo = subItem.Pan2D;
        ...
        sndObj.GetComponent<AudioSource>().time = startTime + subItem.ClipStartTime;

        sndObj.GetComponent<AudioSource>().loop = ( audioItem.Loop == AudioItem.LoopMode.LoopSubitem || audioItem.Loop == AudioItem.LoopMode.LoopSequenceGapless );
        ...
        {
            sndObj.GetComponent<AudioSource>().pitch *= AudioObject.TransformPitch( UnityEngine.Random.Range( -subItem.RandomPitch, subItem.RandomPitch ) );
        }
