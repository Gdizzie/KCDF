...
public class AudioObject_Editor : EditorEx
{
    ...
    private void DrawInspector()
    {
        ...
        ShowString( string.Format( "{0:0.00} half-tones", AudioObject.InverseTransformPitch( AO.GetComponent<AudioSource>().pitch ) ), "Pitch:" );
        if ( AO.GetComponent<AudioSource>() && AO.GetComponent<AudioSource>().clip )
        {
            ShowString( string.Format( "{0} / {1}", AO.GetComponent<AudioSource>().time, AO.clipLength ), "Time:" );
        }
