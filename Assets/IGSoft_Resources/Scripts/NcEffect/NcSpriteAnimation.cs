...
// #endif
	...
	void Update()
	{
		//Debug.Log (this.name + " Lastindex: " + m_nLastIndex);
		...
		if (m_PlayMode != PLAYMODE.RANDOM)
		{
			...
			//Debug.Log (this.name + " nIndex: " + nIndex);
...
//					gameObject.SendMessage(m_funcStartSprite, this, SendMessageOptions.DontRequireReceiver);
			...
			if (m_NcSpriteFactoryCom != null && m_nFrameCount <= 0)
				...
				if ((m_PlayMode == PLAYMODE.PINGPONG ? m_nFrameCount*2-1 : m_nFrameCount) <= nIndex)	// first loop
				{
					if (m_bLoop == false)
					{
						...
						if(AutoPool)
						{
							//Debug.Log (this.name + " PoolEffect");
							//ResetAnimation();
	...
	{
		//Debug.Log (this.name + " SetIndex: " + nIndex);
		if (m_Renderer != null)
		{
			...
			{
				//Debug.Log ("nSetIndex == m_nLastIndex");
				return;
			}
