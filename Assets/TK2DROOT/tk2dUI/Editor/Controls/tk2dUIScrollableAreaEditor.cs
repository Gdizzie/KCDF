...
public class tk2dUIScrollableAreaEditor : Editor
{
    ...
    public void OnSceneGUI()
    {
        ...
        float yAxisVal = isYAxis ? .2f : -.2f;

        float newVisibleAreaLength = tk2dUIControlsHelperEditor.DrawLengthHandles("Visible Area Length", scrollableArea.VisibleAreaLength, scrollableArea.transform.position, isYAxis ? -up : right, Color.red, yAxisVal, 0, .05f);
        ...
        yAxisVal = isYAxis ? .2f : -.2f;

        float doubleAxisVal = isYAxis ? .4f : -.4f;

        float newContentLength = tk2dUIControlsHelperEditor.DrawLengthHandles("Content Length", scrollableArea.ContentLength, scrollableArea.transform.position, isYAxis ? -up : right, Color.blue, yAxisVal, doubleAxisVal, .1f);
