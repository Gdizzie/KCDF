...
public class MB2_MeshCombiner{
	...
	public MB_RenderType renderType { 
		...
		set{
			if (value == MB_RenderType.skinnedMeshRenderer && __renderType == MB_RenderType.meshRenderer){
				if (boneWeights.Length != verts.Length) Debug.LogError("Can't set the render type to SkinnedMeshRenderer without clearing the mesh first.");
			}
			__renderType = value;
		} 
	}
	...
	bool _addToCombined(GameObject[] goToAdd, GameObject[] goToDelete,bool disableRendererInSource){
		...
		if (VERBOSE) Debug.Log("_addToCombined objs adding:" + _goToAdd.Length + " objs deleting:" + _goToDelete.Length + " fixOutOfBounds:" + textureBakeResults.fixOutOfBoundsUVs + " doMultiMaterial:" + textureBakeResults.doMultiMaterial);
		...
		for (int i = 0; i < _goToAdd.Length; i++){
			if(!instance2combined_map.ContainsKey(_goToAdd[i])){
